...
from acm_xml_converter import XML
...
class MainApp(QMainWindow):
    ...
    def convert_file(self):
        ...
        try:
            ...
            # Set BibTeX output style
            use_acm_style = hasattr(
                self, 'acm_bibtex_radio') and self.acm_bibtex_radio.isChecked()
            self.xml_converter.use_acm_style = use_acm_style

            # Set new configuration options
            if hasattr(self, 'use_string_defs'):
                self.xml_converter.use_string_definitions = self.use_string_defs.isChecked()
            if hasattr(self, 'use_biblatex'):
                self.xml_converter.use_biblatex_fields = self.use_biblatex.isChecked()
            if hasattr(self, 'escape_latex'):
                self.xml_converter.escape_latex_chars = self.escape_latex.isChecked()
            ...
            if use_acm_style:
                ...
                self.logger.info("Using ACM BibTeX style")
            ...
            self.logger.error(
                f"Exception during conversion of {file_path}: {str(e)}")
